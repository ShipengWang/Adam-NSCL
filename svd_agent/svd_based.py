...
class SVDAgent(Agent):
    ...
    def update_cov(self, fea_in, k):
        ...
        if len(self.fea_in[k]) == 0:
            self.fea_in_count[k] = fea_in.shape[0]
            self.fea_in[k] = cov / self.fea_in_count[k]
        else:
            fea_in_count = fea_in.shape[0]
            self.fea_in[k] = self.fea_in_count[k] * \
                self.fea_in[k] + fea_in_count * cov
            self.fea_in_count[k] += fea_in_count
            self.fea_in[k] = self.fea_in[k] / self.fea_in_count[k]
    ...
    def init_model_optimizer(self):
        ...
        cls_param = list(cls_params_all[str(self.task_count+1)].parameters())
